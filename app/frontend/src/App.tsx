...
function Pagination({ value }: { value?: PaginationData }) {
  ...
  return <nav className="pagination" aria-label="ページネーション">
    ...
    <div className="page-sizes"><span>表示件数</span>{(value.PageSizes ?? []).map((item) => <Link key={item.Size} to={item.URL} className={item.Selected ? 'selected' : ''} aria-current={item.Selected ? 'page' : undefined}>{item.Size}</Link>)}</div>
  </nav>;
}
function AdminBanner({ children }: { children: ReactNode }) { return <div className="admin-banner"><span>管理者</span>{children}</div>; }
...
const adminNav = [['/admin', '管理概要'], ['/admin/users', 'ユーザー'], ['/admin/keys', '全SSH公開鍵'], ['/admin/subdomains', '全サブドメイン'], ['/admin/tcp-ports', '全TCPポート'], ['/admin/tunnels', 'トンネルとセキュリティ']];
